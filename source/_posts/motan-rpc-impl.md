...
从零开发一款RPC框架，说难也难说简单也简单。难的是你的设计将如何面对实际业务中复杂的应用场景；简单的是其思想可以仅仅浓缩成一行方法调用。`motan`是今年(2016年)新浪微博开源的一款RPC框架，据说在新浪微博正支撑着千亿次调用。对于她的详细介绍和赞誉是新闻稿和README的责任，本文则会从RPC的基本原理入手，逐步分析和讲解motan的各个层次以及其实现思想。

为了避免落入俗套，本文将尽力避免对各种概念的解释（这是wiki的责任），并默认读者已经有了相关RPC框架的使用经验（比如`dubbo`）。为了使得文章充满'干货'，本文重点是讲解`motan`每一部分（层）的实现原理，并配有一定量的代码予以解释。这会使读者不但知晓RPC框架的设计思路，也能知道相关生产代码究竟是如何编写的（在这个浮躁的年代，只有算法和代码是最干的货）。

所有RPC的基本原理几乎一模一样，所以文章先简单解释一下什么RPC，然后直接给出`motan`的整体架构，并解释每层的具体含义。然后我们将从XML的配置入手，逐步讲解provider与consumer的整体流程以及代码调用链。然后详细解析motan的每一个层的设计。
...
- 服务降级是如何做的？
- injvm的协议怎么做到的？
- serialization和codec机制是什么，有哪些内容会被序列化？
- rpc协议的二进制格式到底是个什么样子
...
对于软件工程师来讲，形如`object.method()`的方法调用实在是太熟悉不过，当我们在同一个JVM进程内执行方法调用的时候，一切都显得自然而然。然而，RPC的本质是将上述的调用过程拆分成了两个部分----方法的`调用端`和方法的`实现端`，并将他们分别放置到不同的进程中。调用端和实现端能够做到跨操作系统，跨网络。

从功能角度来讲，RPC框架可以分为服务治理型和多语言型，motan显然属于前者，而其实现的基本思想也不过上面的描述。 因此对motan框架的作用可以简单的理解为：_分离方法调用和实现，并具有一定的双端管理功能_。
...
从上图可以看到，motan的架构层次非常清晰（有点类似dubbo，关于区别请参考[后记](#后记)），从上到下分为6层，其中右侧部分是整个层都需要使用的公共组件。跟其他所有RPC一样，motan框架也有两个核心脉络，其一是`客户端`（对应图左侧垂直箭头），和`服务端`（对应于右侧箭头），服务端不需要进行动态代理，理由很简单，因为服务端有具体的实现，不需要进行切面。最下面的endpoint到endpoint的通信是具体的TCP连接，全双工。
...
- URL，此类非Jdk里的URL，他对协议，路径，参数的统一抽象，系统也是使用URL来保存和读取配置信息的。
- Switcher，开关功能，通过提供开关控制，能够控制框架关键路径的升级和降级功能。
- Exception，统一异常处理，范围业务异常，框架异常，服务异常等。

再来说每一个层的作用：

- Config层，主要提供了配置读取，解析，实体生成。同事他也是整个框架运行的入口，
- Proxy层，服务端无proxy，客户端具有代理功能，他通过InvocationHandler来拦截方法调用。目前只使用了jdk原生动态代理工具。
- Registry，用来进行服务发现和注册，server端进行服务的注册，client进行服务发现。目前有zk,consul的实现，还有对directUrl（就是p2p，不借助中心）的特殊处理。
- Cluster，集群功能，他提供了集群的服务暴露、服务引用、负载均衡，HA等。
- Protocol，协议层，目前只支持injvm和motan协议，主要就是提供给下层协议编码和解析，并且使用filter/pipe模式进行访问日志，最大并发量控制等功能。
- Transport，主要处理网络连接，他抽象了Client和Server接口，使用netty，进行网络链接处理。
...
首先系统需要解析xml配置，将我们自定义的标签配置解析到相关类中（`com.weibo.api.motan.config`）。这里他会读取我们的`<motan:service>`标签，并生成ServiceConfig对象，然后将相关配置解析成这个类的属性。我们这里不需要关心解析过程，因为他实在乏陈。

如何做到xml到对象的映射呢？spring提供了自定义解析机制，这里可以参考相关spring文档，大概分为三步：
...
通过解析，我们完成了从xml到ServiceConfig对象的映射，需要注意的是，每个service标签都会生成一个`ServiceConfig`对象，即所谓的prototype，ProcotolConfig则是使用singleton。

#### 服务export过程

放在spring容器中的bean一定会经过一个初始化过程，ServiceConfig对象也不例外，他会监听应用上下文事件，当整个容器初始化完毕后，调用`export()`方法进行服务暴露。export则主要做了下面几件事情：

1. 将我们第一步解析生成的配置实体类转换成URL类
2. 代理给`ConfigHandler`，生成Exporter对象
...
有了URL配置，我们将暴露过程代理给`ConfigHandler`类
...
可以看到上述暴露过程是先进行服务暴露，然后再注册到中心，通知refer端。原因很明显，如果先通知上线，这个时候服务并没有暴露完毕，refer端实际上是无法调用的。另外，暴露过程又代理给具体的协议实现，这里使用motan协议，即`DefaultRpcProtocol`对象（个人觉得应该叫做MotanProtocol），他实际上又代理给了内部类`DefaultRpcExporter`。在`DefaultRpcExporter`的类初始化中，根据当前系统的SPI配置，找到`EndpointFactory`的实现类，并创建一个Server对象，由server对象的`server.open()`完成transport的具体暴露。

#### transport层服务暴露

motan只实现了基于netty的transport层（可以理解，大部分场景只需要netty即可），在`server.open()`方法中，只是仅仅根据上下文中传递过来的参数，进行bootstrap配置，然后bind端口，对具体端口进行监听。在对bootstrap初始化的时候有两点可能值得注意。一个是我们需要知道他都注入了哪些pipline
...
可以看到，`decoder/encoder`用来解码，编码具体协议；`chanelMessage`是用来进行最大channel并发连接数限制的；`handler`，具体的socket消息处理器，心跳包处理，协议处理的，消息路由等。

另一个是在通常情况下，我们有可能多个provider暴露在相同的端口号下，那么当一个`request`来的时候，具体路由到哪一个provider类上则是由`ProviderMessageRouter`实现，其实现思路比较简单，通过group/interface/version进行provider的区分，然后通过一个`Map<Provider, String>`进行映射即可。

至此，我们的`com.weibo.motan.demo.service.MotanDemoService`服务已在8002端口暴露，关于服务的registry注册请参考后文，[registry机制](#registry机制)。

### 服务调用过程

当一个`Request`到达服务端后，是如何被调用执行的呢？我们知道ProviderMessageRouter可以对`Request`进行路由，当netty的channel收到这个`Request`之后，将会找到具体`Provider`，然后调用`provider.call()`方法进行服务的执行。
...
因此，我们知道，服务端的具体调用也仅仅是`Method.invode()`而已，并没有什么太大的秘密。 至此我们追踪了服务端的两个重要过程：服务暴露、服务调用。如果不配合相关源码的阅读，可能会觉得有些枯燥无味，这里通过一副图来概略说明上述过程。
...
很简单，其实就是要调用远端的MotanDemoService服务。那么具体实现过程是怎么样的呢？

首先与provider端一样，配置解析也需要借助spring框架，但稍有不同，refer端的配置是解析到RefererConfigBean中，他是一个`FactoryBean`，意味着他需要调用`getObject()`获得真正的对象。为什么要这么做？因为我们refer端只是引用了接口，对于接口客户端是无法直接使用的，因此放到spring容器中的一定不是一个接口，他是一个我们代理过的类。因此通过FactoryBean实际上是为了创建代理对象。

接着，在`getObject()`中，会调用子类的`RefererConfig.initRef()`方法，从而进行bean的初始化。

#### cluster初始化

上文的`initRef()`中最重要的当属Cluster的初始化。什么是`Cluster`?他其实代表provider服务集群，每一个具体的provider服务都抽象成一个Referer接口，这个接口与客户端的Exporter相对应。因此，Cluster的本质是`List<Referer>`的容器，并且提供了负载均衡和HA服务（参考下文[cluster集群管理](#cluster集群管理)）。每一个Cluster都被ClusterSupport封装起来，以提供对Cluster的刷新机制和生命周期管理。一个RefererConfig只会对应一个集群。

#### 服务发现与刷新

除了cluster的初始化，`clusterSupport.init()`中还会执行`registry.subscribe(subUrl, this);`已完成服务订阅。所谓的'订阅'，不过是一个逻辑操作，将自己写到zk的服务节点下，实际上的订阅是通过watch机制实现（详见后文[registry机制](#registry机制)）。那么当客户端第一次启动的时候，他是如何感知到的呢？在第一次执行`subscribe`的时候，将会调用`discover()`，然后调用NotifyListener（这里使用的是监听器模式），ClusterSupport实现了notify()接口，从而能够感知到zk中有哪些已经注册的服务以及他们的真实地址。具体的通知过程如下代码：
...
上述代码可以看到，最重要的就是创建新的Referer对象和集群刷新。刷新集群操作会将Cluster旧的不用的Referer对象释放掉，这里采用延迟释放，主要是考虑到Referer可能正在执行中，马上销毁可能影响正常请求。

#### 动态代理

当我们的集群已经准备完毕，并且在zk中订阅了相关服务之后，剩下的工作就只需要进行代理的创建即可。motan中只提供了jdk原生的动态代理实现：
...
不过我们能够通过SPI机制提供自己的实现。我个人觉得搞那么多不同的代理实现方案并没有太大意义，实际工作中基本也只是使用一种而已。

### 调用过程

针对客户端来说，最重要的就是调用过程了，当我们执行一个方法的时候，他实际上是被动态代理的，那么动态代理的入口是哪里呢？`RefererInvocationHandler`提供了这个入口。主要实现如下：
...
下面重头戏来了，`Cluster.call()`实际上代理给了`haStrategy.call()`，然后ha策略对象通过负载均衡`LoadBalance`选择一个或一批Referer对象。然后根据具体策略调用`Referer.call()`。Referer在motan协议默认实现是DefaultRpcReferer，他在初始化的时候通过EndpointFactory创建了一个Client(NettyClient)，然后调用client.request()，从而进入了transport层。

在transport层，NettyClient通过使用commons-pool工具维护了一个连接池缓存，每次调用都通过连接池获得Channel，调用，在释放连接。
...
因此，最后的实现就是`NettyChannel.request()`，他将Request写入到netty的channel中，通过网络发送出去`ChannelFuture writeFuture = this.channel.write(request);`。
...
我们上文说过，在客户端方法调用链中，`Cluster.call()`用于保证服务端高可用，并进行软负载均衡。Cluster实现的本质是Referer对象的容器。见下图：

![motain_cluster](/motan-rpc-impl/motan_cluster.png)

HaStrategy策略接口调用负载均衡器，负载均衡器选择一个Referer交给HaStrategy，然后HaStrategy根据不同策略进行Referer的调用。
...
failover策略是目前最常用的，其实现思路也不难。我们通过负载均衡器选择一组Referer（选择算法根据不同LB有不同实现），然后只调用第一个，当出现错误的时候（非业务异常），我们尝试调用n次（可配置），如果n次都失败了，那么我们就调用下一个Referer，如果这组Referer都调用失败，则抛出异常。

关于loadbalance算法，motan支持以下几种：

- ActivieWegithLoadBalance：最少并发优先，怎么计算并发量？很简单，通过原子计数器即可。
- ConfigurableWeightLoadBalance：可配权重
...
一般RPC框架的协议编码分为两个部分，其一是`协议体`的序列化协议，其二是`协议头`的编码格式。采用固定协议头的好处是容易处理tcp的粘包和半包。motan的协议体默认使用的序列化协议是hession或json，同样我们可以通过SPI进行扩展。那么都有哪些被序列化成协议体（序列化成协议体意味着会在server端进行解析）呢？接口名+方法名+方法参数+方法参数描述以及附加信息（比如接口版本等）
...
Switcher就是开关机制，他提供给整个框架一种降级控制能力。具体的实现主要有一下几个类：
...
即可得到被注入的实现类。如何实现？其主要思路就是通过io操作读取classpath下的相应文件(/META-INF/xxxx)，并进行解析，反射得到接口的实现类，并调用newInstance()创建实例。如果他是单例，就将其缓存，如果不是单例则无需缓存直接返回。@SpiMeta打在接口上，用来标识其生命周期。@Spi打在实现类上，通过name属性作为唯一引用标识，即可打通xml的配置和具体的实现。

## 客户端心跳检测

客户端心跳检测就是定时检测服务端是否可用。为什么要有客户端心跳检测？很明显，对于客户端来讲，服务端是永远不可控的，当出现故障客户端无法调通服务端的时候，除了将该Referer从负载均衡器中拿掉外，还要定时（500ms）发送心跳检测包，以检测服务端是否可用。如下代码：
...
以上都是临时节点，当客户端或服务端的zk会话退出，节点自动销毁。对于客户端还需要watch server目录下的所有child ZNODE，当发生变更，则会调用NotifyListener进行集群的重新刷新。
